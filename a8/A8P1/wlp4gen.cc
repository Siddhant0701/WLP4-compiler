...
void generateReturn(Tree *, Function);
...
{
    generatePrologue();
    generateReturn(t->children[11], procedures[0]);
    generateEpilogue();
}
void generatePrologue()
{
...
void generateReturn(Tree * t, Function f)
{
    Tree * x= t;
    while(t->rule[0] != "ID") t = t->children[0];
    string var = t->rule[1];
