...
void generateWain(Tree *, vector <Function>, Generator*);
void generatePrologue(Generator*);
void generateEpilogue(Generator*);
void code(Tree *, Function, Generator*);
void push(int, Generator*);
void pop(int, Generator*);
...
struct Generator {
    ...
    Generator(): count{1}, ifCount{1} {labels.emplace_back(count);}
    void addStart()
    {
        labels.insert(labels.begin(), ++count);
        code.emplace_back("loop" + to_string(count) + ":");
    }
    void addEnd()
    {
        code.emplace_back("end" + to_string(labels[0]) + ":");
        labels.erase(labels.begin());
    }
    
    int newIf(){return ifCount++;}
    void addIf(int i){code.emplace_back("else" + to_string(i)+":");}
    void addendif(int i){code.emplace_back("endif" + to_string(i) + ":");}
    
    void emplace_back(string s){code.emplace_back(s);}
    ~Generator() 
    { 
        code.clear();
        labels.clear();
...
void generateWain(Tree * t, vector<Function> procedures, Generator * codeGen)
{
...
void generatePrologue(Generator* codeGen)
{
...
void generateEpilogue(Generator* codeGen)
{
    codeGen->emplace_back("add $30, $30, $4");
    codeGen->emplace_back("add $30, $30, $4");
    codeGen->emplace_back("jr $31");
}
void code(Tree * t, Function f, Generator* codeGen)
{
    if(t->rule[0] == "ID")
    {   
        int index;
    ...
    else if(t->rule[0] == "statement")
    {
        if(t->rule[1] == "PRINTLN")
        {
            if(codeGen->code[0] != ".import print") codeGen->code.insert(codeGen->code.begin(), ".import print");
            push(1,codeGen);
            code(t->children[2], f, codeGen);
        ...
        else if(t->rule[1] == "WHILE")
        {
            codeGen->addStart();
            code(t->children[2], f, codeGen);
            codeGen->emplace_back("beq $3, $0, end" + to_string(codeGen->labels[0]));
            code(t->children[5],f,codeGen);
            codeGen->emplace_back("beq $0, $0, loop" + to_string(codeGen->labels[0]));
            codeGen->addEnd();
        }
        else if(t->rule[1] == "IF")
        {
            int x = codeGen->newIf();
            code(t->children[2],f,codeGen);
            codeGen->emplace_back("beq $3, $0, else" + to_string(x));
            code(t->children[5],f,codeGen);
            codeGen->emplace_back("beq $0, $0, endif" + to_string(x));
            codeGen->addIf(x);
            code(t->children[9], f, codeGen);
            codeGen->addendif(x);
        }
        else
        {
    ...
    else if(t->rule[0] == "test")
    {
        ...
        if(t->rule[2] == "LT") codeGen->emplace_back("slt $3, $5, $3");
        else if(t->rule[2] == "GT") codeGen->emplace_back("slt $3, $3, $5");
        else if(t->rule[2] == "LE")
        {
            codeGen->emplace_back("slt $3, $3, $5");
            codeGen->emplace_back("lis $11");
            codeGen->emplace_back(".word 1");
            codeGen->emplace_back("sub $3, $11, $3");
        }
        else if(t->rule[2] == "GE")
        {
            codeGen->emplace_back("slt $3, $5, $3");
            codeGen->emplace_back("lis $11");
            codeGen->emplace_back(".word 1");
            codeGen->emplace_back("sub $3, $11, $3");
        }
        ...
        {
            codeGen->emplace_back("slt $6, $3, $5");
            codeGen->emplace_back("slt $7, $5, $3");
            codeGen->emplace_back("add $3, $6, $7");
        }
        else if(t->rule[2] == "EQ")
        {
            codeGen->emplace_back("slt $6, $3, $5");
...
void push(int a, Generator* codeGen)
{
    codeGen->emplace_back("sw $"+ to_string(a) +", -4($30)");
    codeGen->emplace_back("sub $30, $30, $4");
}
void pop(int a, Generator* codeGen)
{
    codeGen->emplace_back("add $30, $30, $4");
...
int main() {
    ...
    try
    {
        ...
        for (auto a : codeGen->code) cout << a << endl;
